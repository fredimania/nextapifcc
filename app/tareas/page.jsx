...
export default function Page() {
   ...
   setTareas(data)
    //Alternativa axios: const res = await axios.get('/api/tareas');
    // setTareas(res.data)
    }
    ...
    const handleSubmit = async (e) => {
     ...
     //Alternativa axios: const nuevaTareaObj = await 
    axios.post('/api/tareas', { texto: nuevaTarea });
     // const nuevaTareaObj = res.data;
     ...
     //Alternativa axios: await axios.delete('/api/tareas',
 ...
 {tareaEditando && tareaEditando.id === tarea.id ? (
              <button onClick={() => handleEditar(tarea)} className="bg-green-500 text-white p
1 rounded mr-2">Guardar</button>
            ) : (
              <button onClick={() => setTareaEditando(tarea)} className="bg-yellow-500 text
white p-1 rounded mr-2">Editar</button>
            )}
            <button onClick={() => handleEliminar(tarea.id)} className="bg-red-500 text-white 
p-1 rounded">Eliminar</button>
          </li>
        ))}
      </ul>
    </div>
